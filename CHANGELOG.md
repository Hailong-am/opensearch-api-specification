...
- Added an OpenAPI `tags` field to every operation, assigned by semantic domain (e.g. `Document`, `Search`, `Reindex`, `Aliases`, `Index Templates`, `Data Streams`, `Snapshot Policies`), for documentation-renderer sidebar navigation ([#1242](https://github.com/opensearch-project/opensearch-api-specification/pull/1242))
...
### Removed
- Removed `x-distributions-excluded` annotations from spec YAML files and distribution filtering from `OpenApiVersionExtractor`; distribution-specific filtering is now handled by OpenAPI Overlays ([#1219](https://github.com/opensearch-project/opensearch-api-specification/pull/1219))
